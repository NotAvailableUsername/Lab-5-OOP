...
void Benutzer::deletefromWatchlist(Film Filme) {
	...
	for (int i = 0; i < Watchlist.size(); i++) {
		if (Filme.getterTitel() == Watchlist[i].getterTitel()) {
			found = true;
			p = i;
		}
	}
...
void Benutzer::showWatchlist() {
	...
	Repo r(ListaFilme);
	for (int i = 0; i < Watchlist.size(); i++) {
		cout << Watchlist[i].getterTitel() << " " << Watchlist[i].getterGenre() << " " << Watchlist[i].getterJahr() << " " << Watchlist[i].getterTrailer() << " " << Watchlist[i].getterLikes() << " ";
		cout << "\n";
	}
}
